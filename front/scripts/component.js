...
AFRAME.registerComponent('shooting', {
    init: function () {
        ...
        document.body.addEventListener('click', function () {
            if (flag) {
                if (bullet1 > 0) {                                          // 첫번쨰 총으로 쏘면 총알이 10개 장전
                    el.emit('shoot');                                                 
                    magazine01[bullet1].setAttribute('material', 'opacity', '0.2');
                    bullet1 -= 1;
                } else {                                                            // 총알이 없으면 0.5초 후에 10발 장전
                    setTimeout(() => {
                        bullet1 = 10;
                        for (var i = 1; i < 11; i++) {
                ...
                if (bullet2 > 0) {
                    ...
                    setTimeout(() => {                                              // 총알이 없으면 1초 후에 5발 장전
                        bullet2 = 5;
                        for (var i = 1; i < 6; i++) {
        ...
        shooter.addEventListener('componentchanged', function (evt) {               //addEventListener은 이벤트를 등록 -> 오류 발생시 log남김
...
AFRAME.registerComponent('hit-handler', {
    init: function () {
        ...
        el.addEventListener('hit', function () {
            ...
            if (currentActiveBullet === 'bullet01') {                                  // 1번 총이면 -1

                if (damagePoint > 1) {                                                  // 적 생명이 1 이상이면
                    ...
                    setTimeout(() => {                                                      // 1.5초 후에 새로운 enemy 추가하기
